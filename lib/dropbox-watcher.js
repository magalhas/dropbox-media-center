 ...
 * @todo Documentation.
 */
function DropboxWatcher(app, user, options) {
  ...
  this.app = app;
  this.user = user;
  this.dropbox = app.dropbox[user.username];
}
/**
 * @event
...
DropboxWatcher.prototype.run = function () {
  ...
  // First run
  this.beforeWatch(_.bind(this.watch, this));
  return this;
};
/**
 * @fires #onMatch
 ...
 * @todo Documentation.
 */
DropboxWatcher.prototype.scan = function (err, entries, metafolder, metaentries) {
  this.parsedDirs += 1;
  if (err) {
    this.app.error("Error scanning Dropbox folder. Error: %j", err);
    return;
  }
